...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($no_plat === '' || $merek === '') {
        ...
        mysqli_query($koneksi, "UPDATE kendaraan SET no_plat='$no_plat', merek='$merek', tipe='$tipe', tahun='$tahun' WHERE id=$edit_id AND pelanggan_id=$pelanggan_id");
        header("Location: kendaraan.php?pelanggan_id=$pelanggan_id&msg=edit");
        exit();
    } else {
        mysqli_query($koneksi, "INSERT INTO kendaraan (pelanggan_id, no_plat, merek, tipe, tahun) VALUES ($pelanggan_id,'$no_plat','$merek','$tipe','$tahun')");
...
<body class="bg-light">
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4><i class="bi bi-truck"></i> Kendaraan: <span
                    class="text-primary"><?= htmlspecialchars($pelanggan['nama']) ?></span></h4>
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" name="no_plat" class="form-control"
                                    value="<?= htmlspecialchars($edit_data['no_plat'] ?? '') ?>"
                                    placeholder="cth: BP 1234 AB" required>
                            </div>
                            <div class="mb-2">
                                <label class="form-label">Merek <span class="text-danger">*</span></label>
                                <input type="text" name="merek" class="form-control"
                                    value="<?= htmlspecialchars($edit_data['merek'] ?? '') ?>" placeholder="cth: Toyota"
                                    required>
                            </div>
                            <div class="mb-2">
                                <label class="form-label">Tipe</label>
                                <input type="text" name="tipe" class="form-control"
                                    value="<?= htmlspecialchars($edit_data['tipe'] ?? '') ?>" placeholder="cth: Avanza">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Tahun</label>
                                <input type="text" name="tahun" class="form-control"
                                    value="<?= htmlspecialchars($edit_data['tahun'] ?? '') ?>" placeholder="cth: 2021">
                            </div>
                            <div class="d-flex gap-2">
                                <button type="submit"
                                    class="btn <?= $edit_data ? 'btn-warning' : 'btn-primary' ?> btn-sm">
                                    <i class="bi bi-save"></i> <?= $edit_data ? 'Update' : 'Simpan' ?>
                                </button>
                                <?php if ($edit_data): ?>
                                <a href="kendaraan.php?pelanggan_id=<?= $pelanggan_id ?>"
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="kendaraan.php?pelanggan_id=<?= $pelanggan_id ?>&edit=<?= $row['id'] ?>"
                                            class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
                                        <a href="kendaraan.php?pelanggan_id=<?= $pelanggan_id ?>&hapus=<?= $row['id'] ?>"
                                            class="btn btn-danger btn-sm"
                                            onclick="return confirm('Hapus kendaraan ini?')"><i
